...
app.post("/slack/events", async (req, res) => {
  ...
  // URL verification (Slack checks your endpoint during setup)
  if (type === "url_verification") return res.send(req.body.challenge);

  // Handle mentions (in channels) and direct messages (IMs)
  if (
    event &&
    (
      event.type === "app_mention" || 
      (event.type === "message" && event.channel_type === "im" && !event.bot_id) // DM
    )
  ) {
...
User: ${text}`;
    ...
    const post = await slackClient.chat.postMessage({
      channel,
      thread_ts: event.channel_type === "im" ? undefined : rootTs, // threads only in channels
      text: reply
    });
...
// OAuth redirect handler for installation
